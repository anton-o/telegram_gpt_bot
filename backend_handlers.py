...
async def gpt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    model = (
        context.user_data.get("gemini_model", DEFAULT_GEMINI_MODEL)
        if use_gemini
        else context.user_data.get("oai_model", DEFAULT_OAI_MODEL)
    )
...
async def set_current_model(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            if use_gemini:
                ...
                async for model_obj in pager:
                    m_name = (
                        model_obj.name.lstrip("models/") if model_obj.name else None
                    )
            ...
            await update.message.reply_text(f"⚠️ Failed to fetch the model list: {e!s}")
        return
    ...
    if use_gemini:
        ...
        await update.message.reply_text(
            f"✅ Gemini model changed from {current_model} to {new_model}"
        )
    else:
        context.user_data["oai_model"] = new_model
        await update.message.reply_text(
            f"✅ OpenAI model changed from {current_model} to {new_model}"
        )
...
async def _async_ask_llm(user_request: str, use_gemini: bool, model: str) -> str:
    try:
        if use_gemini:
                ...
                if metadata.grounding_chunks:
                    for chunk in metadata.grounding_chunks:
                        ...
                        if hasattr(chunk, "web") and chunk.web and chunk.web.uri:
                            # Keying by URI deduplicates identical source links.
                            source_links[chunk.web.uri] = (
                                chunk.web.title or "Source Link"
                            )
            ...
            return "oai: " + (
                response.choices[0].message.content or "[No text returned]"
            )

    except Exception as e:
        return f"⚠️ API Error: {e!s}"
